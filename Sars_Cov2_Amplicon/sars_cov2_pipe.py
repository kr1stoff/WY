...
def get_args():
    ...
    parser.add_argument("--global_sars2", dest="gsars2",
                        action="store_true", help="是否使用新冠全球库进行分析, 默认不运行.")
...
{self.fastp} --thread {self.thread} {self.options_fastp} \
    --adapter_fasta {self.home}/etc/adapter.fa \
...
echo fastp时间2: $(date)
{self.python} {self.home}/bin/parse_fastp.json.py \
...
{self.Rscript} {self.home}/bin/genome_coverage.R \
...
{self.bedtools} genomecov -bga -ibam {self.dict_dirs['map']}/{self.prefix}.bam \
    ...
    def gatk_variant_pipe(self):
        ...
        self.cmds.append(f"{self.python} {self.home}/bin/gatk_filter.py "
    ...
    def variant(self):
        ...
        # gatk流程需要.fai和.dict
        self.cmds.append(
            f"{self.samtools} faidx {self.dict_dirs['consensus']}/{self.prefix}.masked.fa")
        self.freebayes_variants()
        ...
        self.cmds.append(
            f"ln -sf {self.prefix}.snpEff.vcf {self.dict_dirs['variant']}/{self.prefix}.vcf")
        ...
        # 过滤VCF,并转为table格式
        self.cmds.append(f"{self.python} {self.home}/bin/vcf2table.py "
    ...
    def consensus(self):
        ...
        self.cmds.append(
            f"sed -i 's/>.*/>{self.prefix}/g' {self.dict_dirs['consensus']}/{self.prefix}.consensus.fa")

    def single_tree(self):
        logging.info("单样本进化树")
        self.cmds.append("# 单样本进化树")
        self.cmds.append(f"cat {self.dict_dirs['consensus']}/{self.prefix}.consensus.fa "
                         f"{self.home}/etc/SARS-CoV-2_isolates13.fasta "
        ...
        self.cmds.append(
            f"{self.Rscript} {self.home}/bin/tree4SC2pipe.R {self.dict_dirs['phylogenetic']}/{self.prefix}.tre")
        self.cmds.append(
            f"{self.python} {self.home}/bin/magick.py {self.magick} {self.dict_dirs['phylogenetic']}")

    def global_sars2(self):
        logging.info("新冠全球库")
        if self.gsars2:
            self.cmds.append("# 新冠全球库")
            self.cmds.append(f"{self.python} {self.home}/bin/sars_cov2_global.py "
    ...
    def make_result_dirs(self):
        ...
        # !顺序不要变
        # , "5.phylogenetic"] # [220628 update] 一分钟的力量,弃用
