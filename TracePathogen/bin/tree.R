...
if (file.info(tree_file)$size == 0) {
  ...
  system(paste0("touch ", output_dir, "/Failed_to_create_phylogenetic_tree"),
        intern = TRUE)
}
...
p1 <- ggtree(tree, layout = "rectangular",
  ...
  geom_text(aes(x = branch, label = branch_len_format, vjust = -.3),
          color = "black")
ggsave(plot = p1, filename = "rectangular.svg",
       width = p124_width, height = graph_height, path = output_dir)
...
ggsave(plot = p2, filename = "rectangular_bl.svg",
       width = p124_width, height = graph_height, dpi = 300, path = output_dir)
...
p3 <- ggtree(tree, layout = "circular",
  ...
  geom_tippoint(color = "black", alpha = 1 / 4) +
  geom_nodepoint(color = "black", alpha = 1 / 4)
ggsave(plot = p3, filename = "circular.svg", width = p3_width,
      height = p3_width, dpi = 300, path = output_dir, limitsize = FALSE)
