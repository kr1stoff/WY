...
# @Last Modified time: 2023/8/8 15:11
import logging
import sys
...
@click.option('--merge',
              required=True,
              type=click.Path(),
              help="The primer.xls generate by pipe")
@click.option('--snp',
              required=True,
              type=click.Path(),
              help="The all.variants.bed generate by the pipe")
@click.option('-o', '--out',
              required=True,
              type=click.Path(),
...
def main(merge, snp, out, freq):
    ...
    *注意流程中的突变文件是有一定的筛选条件的，如果freq低于流程中的筛选条件，可按照freq=0的条件重新生成all.variants.bed文件*
    ...
    logger.info(f"Parse the Sequence Primer result file {f_merge}")
    ...
    with open(f_merge, 'r') as IN, open(f_out, 'w') as OUT:
        ...
        for line in IN:
            ...
            chromosome = arr[1]
            # 内引物F
            start_forward_internal = arr[2]
            seq_forward_internal = arr[7]
            arr[7] = seq2degenerate(seq_forward_internal, chromosome, start_forward_internal, info_snp)
            # 内引物R
            start_reverse_internal = arr[8]
            seq_reverse_internal = arr[13]
            arr[13] = seq2degenerate(seq_reverse_internal, chromosome, start_reverse_internal, info_snp, strand='-')
