...
# @Last Modified time: 2023/8/8 9:14
import logging
...
@click.option('--info',
              required=True,
              type=click.Path(),
              help="The sequence primer design result")
...
def main(specificity, inclusive, info, species, out):
    ...
    合并测序引物的设计及评估结果
    """
    f_spe = Path(specificity).absolute() if specificity else None
    ...
    if f_spe:
        ...
        with open(f_spe, 'r') as IN:
            header_spe = ["Specificity F Number(percent %)", "Specificity R Number(percent %)",
                          "Specificity Pair Number(percent %)"]
            for line in IN:
                arr = line.strip().split("\t")
                info_spe[arr[0]] = [f"{arr[4]}({arr[3]})", f"{arr[6]}({arr[5]})", f"{arr[2]}({arr[1]})"]
    else:
        header_spe = []
        info_spe = {}
    ...
    with open(f_inc, 'r') as IN:
        header_inc = next(IN).strip().split("\t")[3:]
        for line in IN:
            arr = line.strip().split("\t")
            info_inc[arr[0]] = arr[3:]

    logger.info(f"Out put the result to file {f_out}")
    with open(f_out, 'w') as OUT:
        header = ["Species"] + header_primer + header_spe + header_inc if len(header_spe) > 0 else [
                                                                                                       "Species"] + header_primer + header_inc
        print(*header, sep="\t", file=OUT)
        for i, j in info_primer.items():
            arr = [species, *j, *info_spe[i], *info_inc[i]] if len(info_spe) > 0 else [species, *j, *info_inc[i]]
