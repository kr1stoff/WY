...
def merge_vcf(info: list, freq: float):
    ...
    :return res: A dict contain all mutations and their count
    """
    ...
    for i, j in tmp.items():
        if len(j) > num_mut_cutoff:
            snp_type = list(set(j))
            res[i] = [snp_type, [str(j.count(snp)) for snp in snp_type]]
    return res
...
def main(din, fout, freq):
    ...
    with open(f_out, 'w') as OUT:
        for i, j in sorted(vcf.items(), key=lambda x: x[0][1]):
            info_snp = ['|'.join(x) for x in j[0]]
            print(*[*i, ','.join(info_snp), ','.join(j[1]), len(info_vcf) + 1, "+"], sep="\t", file=OUT)
