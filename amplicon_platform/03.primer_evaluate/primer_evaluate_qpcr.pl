...
my $primer_blast_dir = abs_path("$Bin/../primer_blast");
my $conservative_dir = abs_path("$Bin/../conservative");
my $common_bin = abs_path("$Bin/../common_bin");
...
open PRI,$primerfile || die $!;
open TABLE_PRIMER,">$outdir/primer.table.taxid" || die $!;
open BED,">$outdir/in.product.bed" || die $!;
open TABLE_PROBE,">$outdir/probe.table.taxid" || die $!;
my %hash_fa;
my ($taxid,$spe_name);
while(<PRI>){
    chomp;
...
my $outfotmat_inc = "\'6 qaccver saccver pident length mismatch gapopen qstart qend sstart send evalue bitscore qlen slen qcovs staxid sscinames sstrand\'";
...
if ($specificity eq "T"){
    ...
    if ($blast eq "T"){
        ...
        $cmd_spe .= "# python {Bin}/primer_blast/blast_nt_cut.py --fasta $outdir/primer.fa --nt_database $nt_database --out $outdir/blastn.nt.out\n";
        my $st = "# 无需nt库比对, 直接读取 $workdir1/blastn.nt.out\n";
    ...
    ### probe ###
    $cmd_spe .= "### probe ###\n";
    $cmd_spe .= "mkdir -p $workdir1/02.probe\n";
    $cmd_spe .= "perl $primer_blast_dir/split_workdir.pl $outdir/probe.table.taxid $workdir1/blastn.nt.out.filter 02.probe\n";
    ###
    $cmd_spe .= "cat $outdir/probe.table.taxid | while read -r id p taxid; do perl $primer_blast_dir/judge_primer_blast.pl $taxid_list 02.probe/\$id/\$id.blast > 02.probe/\$id/\$id.blast.same; done\n";
    $cmd_spe .= "cat $outdir/probe.table.taxid | while read -r id p taxid; do perl $primer_blast_dir/same_stat.pl 02.probe/\$id/\$id.blast.same > 02.probe/\$id/\$id.blast.same.stat; done\n";
    $cmd_spe .= "cat $outdir/probe.table.taxid | while read -r id p taxid; do perl $conservative_dir/probe_blast_stat.pl 02.probe/\$id/\$id.input.table 02.probe/\$id/\$id.blast.same.stat > 02.probe/\$id/\$id.all.stat; done\n";
    ### 合并结果
    $cmd_spe .= "if [ -f \"probe.all.stat\" ]; then \n rm probe.all.stat \nfi\n";
    $cmd_spe .= "cat $outdir/primer.table.taxid | while read -r id f r taxid; do cat 02.probe/\$id/\$id.all.stat | grep -v '#' >> probe.all.stat; done\n";

    $cmd_spe .= "perl $primer_blast_dir/merge_all_spe_stat.pl primer.all.stat probe.all.stat $outdir/probe.table.taxid > specificity.all.stat\n";
...
if ($inclusiveness eq "T"){
    ...
    $cmd_inc .= "cd $workdir2\n";
    $cmd_inc .= "$blastn -task blastn-short -query $outdir/primer.fa -db $genome_dir/all -num_threads 60 -word_size 9 -evalue 1000 -max_target_seqs 99999 -qcov_hsp_perc 85 -perc_identity 85 -penalty -1 -reward 1 -outfmt $outfotmat_inc -out $workdir2/blastn.genome.out\n";
    ...
    ###############
    ### probe ###
    $cmd_inc .= "mkdir -p $workdir2/02.probe\n";
    $cmd_inc .= "perl $primer_blast_dir/split_workdir.pl $outdir/probe.table.taxid $workdir2/blastn.genome.out.filter 02.probe\n";
    ###
    $cmd_inc .= "cat $outdir/probe.table.taxid | while read -r id p taxid; do perl $conservative_dir/epcr_result_deal.pl $genome_dir/assembly_chromosome.tsv 02.probe/\$id/\$id.blast 02.probe/\$id/\$id.assembly_chromosome.copy 02.probe/\$id/\$id.blast.chr 02.probe/\$id/\$id.blast.stat ; done\n";
    $cmd_inc .= "cat $outdir/probe.table.taxid | while read -r id p taxid; do perl $conservative_dir/probe_blast_stat.pl 02.probe/\$id/\$id.input.table 02.probe/\$id/\$id.blast.stat > 02.probe/\$id/\$id.all.stat ; done\n";
    ### 合并结果
    $cmd_inc .= "if [ -f \"probe.all.stat\" ]; then \n rm probe.all.stat \nfi\n";
    $cmd_inc .= "cat $outdir/primer.table.taxid | while read -r id f r taxid; do cat 02.probe/\$id/\$id.all.stat | grep -v '#' >> probe.all.stat; done\n";
    $cmd_inc .= "perl $conservative_dir/merge_all_con_stat.pl primer.all.stat probe.all.stat $outdir/probe.table.taxid > inclusiveness.all.stat\n";
...
if ($specificity eq "T" and $inclusiveness eq "T"){
    ...
    $cmd_stat .= "$bedtools getfasta -fi $genome_dir/ref.fna -fo in.product.bed.fasta -bed $outdir/in.product.bed\n";
    $cmd_stat .= "perl $Bin/stat/merge_all_stat_qpcr.pl $outdir/01.specificity/specificity.all.stat $outdir/02.inclusiveness/inclusiveness.all.stat $primerfile $taxid in.product.bed.fasta\n";
