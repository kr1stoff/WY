...
while(<A>){
    ...
    if ($strand eq "plus"){
        ...
        if ($query eq "$primer2"){
            # PRIMER-2
            $hash_posi_positiveR{$seqid} .= "$query:$sstart_new:$send:$mismatch:$gap:$qstart:$taxid"."___";
        }
        else{
            # PRIMER-1
            $hash_posi_positiveF{$seqid} .= "$query:$sstart_new:$send:$mismatch:$gap:$qstart:$taxid"."___";
        }
    }
        ...
        if ($query eq "$primer2"){
            # PRIMER-2
            my $sstart_new = $send;
            $hash_posi_negativeR{$seqid} .= "$query:$sstart_new:$send_new:$mismatch:$gap:$qstart:$taxid"."___";
        }
        else{
            # PRIMER-1
            my $sstart_new = $send;
            $hash_posi_negativeF{$seqid} .= "$query:$sstart_new:$send_new:$mismatch:$gap:$qstart:$taxid"."___";
        }
    }
}
close A;
...
foreach my $id (sort keys %hash_posi_positiveF){
    if (exists $hash_posi_negativeR{$id}){
        ...
        my @lists1 = (split /___/,$hash_posi_positiveF{$id});
        my @lists2 = (split /___/,$hash_posi_negativeR{$id});
...
foreach my $id (sort keys %hash_posi_positiveR){
    if (exists $hash_posi_negativeF{$id}){
        ...
        my @lists1 = (split /___/,$hash_posi_positiveR{$id});
        my @lists2 = (split /___/,$hash_posi_negativeF{$id});
        foreach my $posi1 (@lists1){
            my ($primer1,$start1,$end1,$mismatch1,$gap1,$qstart1,$taxid) = (split /:/,$posi1)[0,1,2,3,4,5,6];
            foreach my $posi2 (@lists2){
                # print "Test\t$posi1\t$posi2\n";
