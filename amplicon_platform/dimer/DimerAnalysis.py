...
def run_mfeprimer(fasta, mfeprimer, out):
    """ 运行mfeprimer二聚体分析 """
    suffix = Path(fasta).absolute().stem
    outfile = Path(out).absolute().joinpath(f'{suffix}.mfeprimer')

    cmd = f"{mfeprimer} dimer -c 32 --dg -3 -i {fasta} -o {outfile} "
    logging.info(f'运行命令 {cmd}')
    run(cmd, shell=True, capture_output=True, encoding="utf-8")
    logging.info(f'输出 {outfile}')
    return outfile
...
def parse_group_list(group_list: list):
    ...
    # 输出,筛选 符合二聚体和二聚体长度 自由能阈值
    if flag == 'Y' and (len_align < len(seq1)*0.7 or len_align < len(seq2)*0.7):
        return [idx, f'{p1}::{p2}', score, tm, delta_g, seq1, seq2, len(seq1), len(seq2), len_align, mismatch]


def parse_file(file, out):
    """遍历mfeprimer结果"""
    basename = Path(file).absolute().name
    outfile = Path(out).absolute().joinpath(f'{basename}.filter')
    OUT = open(outfile, 'w', encoding='utf-8')
    print("#index\tdimer\tscore\ttm\tdelta_g\tseq1\tseq2\tlen1\tlen2\tlength\tmismatch\tflag", file=OUT)
    ...
    with open(file, 'r', encoding='utf-8') as f:
        ...
        for line in f:
            ...
            if len(group_list) == 7:
                res = parse_group_list(group_list)
                if res:
...
# =========================================================================================
Path_mfeprimer = '/home/yehui/software/bin/mfeprimer'
...
@click.option('-i', '--fasta',
              required=False,
              type=click.Path(),
              help="输入需要分析二聚体的fasta")
@click.option('-p', '--mfeprimer',
              required=False,
              default=Path_mfeprimer,
              type=click.Path(),
              help=f"mfeprimer路径,默认 {Path_mfeprimer}")
@click.option('-o', '--out', required=False,
              default='./',
              type=click.Path(),
              help=f"结果输出文件夹,[./]")
def main(fasta, mfeprimer, out):
    """
    输入fasta序列, 运行 mfeprimer, 解析软件输出结果
    [输出格式]: 序号 二聚体 得分 tm值 自由能 序列1 序列2 序列1长度 序列2长度 overlap长度 错配 是否为二聚体
    """
    outfile = run_mfeprimer(fasta, mfeprimer, out)
    parse_file(outfile, out)
