...
# @Last Modified time: 2023/12/06 15:03
import logging
import subprocess
import sys
...
class wgSNP(object):
    ...
    def __init__(self, sample, ref, out):
        """
        Init the object
...
snippy-core --ref {self.f_ref} {' '.join(self.sample)}
...
{self.software['rscript']} {BINPATH}/tree.R --branchlength --tree {self.d_gubbins}/gubbins.final_tree.tre --prefix wgSNP
"""
            print(cmd, file=OUT)
    ...
    def to_upload(self, d_upload):
        ...
        """
        cmd = f"""set -e
cp -rf {self.d_out}/gubbins/gubbins.final_tree.tre {d_upload}/wgSNP.tre
cp -rf {self.d_out}/plot/wgSNP.png {d_upload}/wgSNP.png
"""
...
def cli(sample, reference, out, run):
    """
    wgSNP流程
    """
    ...
    project = wgSNP(f_sample, f_reference, d_out)
    project.prepare()
    project.fastp()
    project.snippy()
    project.post_snippy()
