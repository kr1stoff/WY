...
# @Last Modified time: 2023/12/26 16:46
import csv
import getopt
import itertools
import logging
...
dico_comp = {'A': 'T', 'C': 'G', "G": "C", "T": "A", "M": "K", "R": "Y", "W": "W", "S": "S", "Y": "R", "K": "M",
             "V": "B", "H": "D", "D": "H", "B": "V", "X": "X", "N": "X", ".": ".", "|": "|"}
dico_degenerate = {'R': 'AG', 'Y': 'CT', 'S': 'CG', 'W': 'AT', 'K': 'GT', 'M': 'AC', 'B': 'CGT', "D": 'AGT', 'H': 'ACT',
                   "V": "ACG", "N": "ACGT", ".": ""}
...
def pretty_mismatch(primer, found, nb_mismatch):  # lower the mismatches nucleotides
    ...
    if len(found) == len(primer) and nb_mismatch >= len(
            [e for e, i in zip(primer, found) if e != i]):  # only mismatch(s)
...
def mismatches(nb, primer, seq, nbmismatch):
    ...
    if nb == 1:
        ...
        if tmpfind == []:
            tmpfind = search_matches(nbmismatch, invP,
                                     seq)  # get the match(es) with mismatches (use of regex.findall())
            tmpfind = remove_redondant_matches(tmpfind, primer)
        ...
        if primers:  # if primers had been entered
            for p, primer in enumerate(primers):  # for each couple of primers
                ...
                for primer1 in primers_1:
                    tmp1_handle, tmp2_handle, mismatchs_handle, len_mismatch_handle = findFirst(primer1, seq,
                                                                                                nbmismatch)
                ...
                # print (tmp1)
                if nbmismatch > 0 and mismatchs != []: mismatchs = clean_mismatches(1, primer[1], tmp2, mismatchs,
                                                                                    nbmismatch)
                ...
                for i, pos_match in enumerate(first_match[0]):  # for each match of the first primer
                    ...
                    for primer2 in primers_2:
                        tmp_handle, mismatchs2_handle, len_mismatch2_handle = findSec(primer2, seq, first_match[1][i],
                                                                                      nbmismatch)
                    ...
                    if nbmismatch > 0 and mismatchs2 != []: mismatchs2 = clean_mismatches(2, primer[2],
                                                                                          [first_match[1][i]] * len(
                                                                                              tmp), mismatchs2,
                                                                                          nbmismatch)  # lower the mismatched nucleotides
                    if tmp != []:  # if there is a match with the second primer on the complementary DNA sequence
                        for m, pos_match2 in enumerate(tmp):  # for each match found for the second primer
                            ...
                            if first_match[1][i] == "inv":
                                ...
                                size2 = pos_match + len(primer[1]) + (len(seq) - (pos_match2 + len_match2 - len(primer[
                                                                                                                    2])))  # if primers are separated by the splitted area in the sequence (circular chromosome)
                            else:
                                size = pos_match2 + len(primer[2]) - (pos_match + len_match - len(primer[1]))
                                size2 = pos_match2 + len(primer[2]) + (
                                        len(seq) - (pos_match + len_match - len(primer[1])))
                            ...
                            if size > 0:
                                ...
                                if splitted is True:  # if insert is separated by the splitted area in the sequence (circular chromosome)
                                    if pos_match < pos_match2:
                                        insert = inverComp(
                                            seq[pos_match2:] + seq[:pos_match + len(primer[1])])  # reversed comp insert
                                    else:
                                        insert = seq[pos_match:] + seq[:pos_match2 + len(primer[2])]
                                else:
                                    if pos_match < pos_match2:
                                        insert = seq[pos_match:pos_match2 + len(primer[2])]
                                    else:
                                        insert = inverComp(
                                            seq[pos_match2:pos_match + len(primer[1])])  # reversed comp insert
                                ...
                                if binning is True and primer[
                                    0] in dico_bin:  # if option binning selected, correction with the dictionary
                                    sizeU = binning_correction(primer[0], size, sizeU)

                                if sizeU < 100: result.append(
                                    [primer[0], pos_match, pos_match2, size, sizeU, sequence + str(s + 1), nbmismatch,
                                     primer[1], mismatch, primer[2], mismatch2, insert])
                ...
                if len(result) == 0 and primer_info[0] not in dico_res:  # if no result
                    ...
                    if primer_info[0] in dico_res and dico_res[primer_info[0]][4] != "":
                        best_res[5] = best_res[5] + ", " + sequence + str(
                            s + 1)  # if there's already a result with perfect matches
                    dico_res[primer_info[
                        0]] = best_res  # set the best result as a new key : value in the dictionnary #replace the old dictionnary value if there is one
                    if flanking is True:
...
def main():
    ...
    try:  # check if correct args
        opts, args = getopt.getopt(sys.argv[1:], "hm:i:o:p:cr:b:f:",
                                   ["help", "mismatch=", "input=", "output=", "primer=", "contig", "round=" \
    ...
    for i, file in enumerate(files):
        logger.info(f"{file}\tstrain{i + 1}/{len(files)}")
        fasta = open(fasta_path + file, "r").read()
        fasta_names = []
        for line in fasta.split("\n"):
            if ">" in line:
                if '|' in line and len(line.split("|")) >= 5:
                    # for fasta names like : >gi|1032812322|ref|NZ_CP015344.1| Legionella pneumophila strain D-7630, complete genome
                    tmp_names = line.replace("\n", "").split("|")
                    fasta_names.append([tmp_names[3].split(".")[0], tmp_names[4][1:].split(",")[0]])
                    del tmp_names
                else:
                    # for fasta names like : >NC_003317.1 Brucella melitensis bv. 1 str. 16M chromosome I, complete sequence
                    tmp_names = line.replace("\n", "").split(",")[0].split(" ")
                    fasta_names.append([tmp_names[0].replace(">", ""), " ".join(tmp_names[1:])])
        if len(fasta_names) == 0:
            logger.error(f'no fasta header found in {file}')
            sys.exit()

        if mixte is True:
            # check for each fasta if there is more than one sequence (contigs) or not with --mixte option
            if fasta.count(">") > 1:
                contig = True
                sequence = "contig"
            else:
        ...
        if i == 0:
            output_file = []
            header = ["strain", "primer", "position1", "position2", "size", "allele", "sequence", "nb_mismatch",
                      "primer1", "mismatch1", "primer2", "mismatch2", "predicted PCR target"]
            if flanking is True:
                header.extend(['flanking1', "flanking2"])
            out = csv.writer(open(output_path + fasta_path.split("/")[-2] + "_output.csv", "w", encoding='utf-8'),
                             delimiter=',', quoting=csv.QUOTE_NONE)
            for row in [header]:
                out.writerow(row)
            out = csv.writer(open(output_path + fasta_path.split("/")[-2] + "_output.csv", "a", encoding='utf-8'),
                             delimiter=',', quoting=csv.QUOTE_NONE)
        ...
        for Primer in Primers_short:
            ...
            if flanking is True:
                if Primer in dico_flanking:
                    cr.append([file] + result[Primer] + dico_flanking[Primer])
                else:
        ...
        for primer_short, primer_full in zip(Primers_short, Primers_full):
            if full_locus_name is False:
                locus.append(primer_short)
            else:
                locus.append(primer_full)
    ...
    logger.info(f"MLVA analysis finished for {fasta_path.split('/')[-2]}")
    ...
    dico_mismatch = {key: value for key, value in dico_mismatch.items() if
                     value != set()}  # delete keys without value(s)
    ...
    for primer in Primers:
        if primer[0] + "_FOR" in dico_mismatch:
            tmp_file += primer[0] + "_FOR\n" + primer[1] + "\n" + "\n".join(
                list(dico_mismatch[primer[0] + "_FOR"])) + "\n\n"
        if primer[0] + "_REV" in dico_mismatch:
            tmp_file += primer[0] + "_REV\n" + primer[2] + "\n" + "\n".join(
                list(dico_mismatch[primer[0] + "_REV"])) + "\n\n"
...
if __name__ == "__main__":
    """
    modified from https://github.com/dpchris/MLVA.git
    """
    main()
