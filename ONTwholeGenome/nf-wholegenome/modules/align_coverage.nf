process MAP_DEPTH {
  ...
  output:
  tuple val(sample_id), path("sort.bam.depth")
  
  script:
  """
  samtools depth -a $bam > sort.bam.depth
  """
}
...
process COVERAGE {
  ...
  output:
  tuple val(sample_id), path("coverage.tsv"), path("coverage.png")
  ...
  # 1.覆盖的统计表
  # 分类数据
  df = pd.read_table("$depth", sep='\\t', header=None)
