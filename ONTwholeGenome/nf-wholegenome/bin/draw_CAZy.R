...
if (length(args) != 2) {
  usage = "Usage:   Rscript draw_CAZy.R <CAZY.kind_stat.txt> <out_file>\n"
  cat(usage)
  quit("no")
}

in_file <- args[1]
out_file <- args[2] # CAZy_anno_stats.png
...
ggplot(df, aes(x = kind , y = frequency , fill = kind)) +
    geom_bar(stat = "identity", position = position_dodge(0),width =0.3,colour="black",linewidth=0.2 )+
