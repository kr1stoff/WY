...
prefix = os.path.abspath(sys.argv[3])       # detail_drug_resistance.txt
...
if os.path.exists(infile) and os.path.exists(depfile):
    ...
    total_df.to_excel(f"{prefix}.xlsx",index=False)
    total_df.to_csv(f"{prefix}.txt",sep='\t',index=False,encoding='utf-8_sig')
