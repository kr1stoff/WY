...
if (-s  "${dir}/1.qc/abundance.txt"){
    $section->submenu("reads物种注释丰度统计",-icon => "./src/image/icon/2.3depollute.png");
    $section->desc("$blank6 将所有reads进行快速的物种注释，统计出reads中的物种丰度信息，如下表：");
    $section->tsv2html(
...
if (-e "${dir}/2.ass/${sample}_scaffolds.fasta"){
    ...
    $section->desc("$blank6 组装完成后，过滤掉长度 <500bp 的序列。");
    ...
    if (-e "$andir/3.pre/predict/predict.gff"){
        ...
        if (-s "${dir}/3.pre/TandemRepeat.txt"){
            $section->tsv2html(
        ...
        if (-s "${dir}/4.anno/Virulence.txt"){
            ...
            $section->desc("说明：<br>Related Genes：相关毒力基因简称；Virulence Factors：毒力因子；Description：描述；Speices：相关物种；VFcategory：类别；Characteristics：特征；Structure：结构；Function：功能；Mechanism：机制");
        }else{
            $section->desc("$blank6 无毒力基因注释结果");}
