...
class STAT():
    ...
    def length_plot(self):
        ...
        colors_list = cm.tab20(gradient)  # 渐变

        # plt.figure(figsize=(15, 8),dpi=300)       
        plt.figure(figsize=(len(labels)//2, len(labels)//4),dpi=300)    
        # p1 = plt.bar(range(len(df)),df,color="#4d97cd")
        p1 = plt.bar(range(len(df)),df,color=colors_list, alpha=0.7)
        plt.xlabel('Length of scaffolds(bp)',size=14)
        plt.ylabel('Number of reads',size=14)
        plt.xticks(rotation=45,ticks=num_ticks,labels=word_ticks)    #坐标倾斜 ,并自定义横坐标
        plt.tick_params(labelsize=8)
        plt.ylim([0, max_count*1.1])        #限制纵坐标
        plt.bar_label(p1,label_type='edge',size=11)    #数据标签，edge表示将数据值标签放在柱子顶端
        ...
        logging.info(f"[Output]: {self.out}/{self.prefix}.length.png")
