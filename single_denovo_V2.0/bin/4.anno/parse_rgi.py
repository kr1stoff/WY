...
class Rgi():
    def __init__(self,**kwargs):
        ...
        self.depcov = os.path.abspath(kwargs['depcov']) if kwargs['depcov'] else ''
    ...
    def parse_depcov_file(self):
        if self.depcov:
            self.data = pd.read_csv(self.depcov,sep='\t',encoding='utf-8',header=0)
            self.data["Depth"] = self.data['Depth'].apply(lambda x: '{:.2f}'.format(x))
            self.data["Coverage"] = self.data['Coverage'].apply(lambda x: '{:.2f}'.format(x))
...
@click.option('-1','--file', required=True,type=click.Path(),help="输入rgi.txt文件")
