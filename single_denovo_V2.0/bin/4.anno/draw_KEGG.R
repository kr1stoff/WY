...
ggplot(KEGG_table, aes(x = V2 , y = V3 , fill = V1)) +
  ...
  geom_text(aes(label = V3 , y = V3 ), size=6, hjust = -0.4, position = position_dodge(0.5))+   # 数据标签
  #去除绘图区和X轴之间的gap
  ...
  theme(legend.title=element_blank(), # 图例标题
        ...
        legend.text = element_text(size = 20),   # 图例字体
        legend.key.size = unit(35, "pt"),       # 图例大小
        panel.grid = element_blank(),           # 网格线
        axis.text = element_text(size = 20),     # 刻度标签
