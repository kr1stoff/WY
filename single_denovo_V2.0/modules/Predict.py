...
class Predict(Pipe):
    ...
    def creat_env(self):
        ...
        os.makedirs(self.p_repeat,exist_ok=True)
...
cd {self.p_repeat}
rm -rf {self.p_repeat}/*
time RepeatMasker -e ncbi {self.infa} -dir {self.p_repeat} -pa 32 -s -nolow -gff -lib {params.db_repeatMasker}
{params.python3} {BIN}/3.pre/ParseRMresult.py -2 {self.p_repeat}/{self.id}_scaffolds.fasta.out.gff --anno {params.db_repeatMasker_tab} -o  {self.p_repeat}/INEs.txt

# trf
trf {self.infa} 2 7 7 80 10 50 500  -h -m -ngs >{self.p_repeat}/TandemRepeat.dat
{params.python3} {BIN}/3.pre/parse_TrfDat.py -i {self.p_repeat}/TandemRepeat.dat
...
if [ -f "{self.p_predict}/predict.gff" ]; then
    ...
    paste {self.p_predict}/bed_bam.merge.depth {self.p_predict}/bed_bam.merge.cov |cut -f 4,5,13 |sed '1i Contig\\tDepth\\tCoverage' >{self.merge_file}
