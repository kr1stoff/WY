...
if os.path.exists(infile) and os.path.exists(depfile):
    ...
    col=["Contig","ARO","Name","Best_Hit_Bitscore","Best_Identities","Percentage Length of Reference Sequence","Drug Class","Resistance Mechanism",]
    #选取部分列
    ...
    #列标题重命名
    df.columns = ["比对基因ID","耐药基因ID","耐药基因简称","比对得分","置信度(%)","覆盖度(%)","耐药基因分类","作用机制"]  
    #排序
    ...
    total_df.to_excel(f"{outdir}/detail_drug_resistance.xlsx",index=False,encoding='utf-8')
    total_df.to_csv(f"{outdir}/detail_drug_resistance.txt",sep='\t',index=False,encoding='utf-8')
