...
elif [ -f "{self.p_fastqc}/{self.id}_fastqc.html" ];then 
    cp -r {self.p_fastqc}/{self.id}_fastqc {self.p_report}/1.qc
    cp -r {self.p_fastqc}/{self.id}_fastqc.html {self.p_report}/1.qc

    cp -r {self.p_filter_fastqc}/{self.id}_clean_fastqc {self.p_report}/1.qc
    cp -r {self.p_filter_fastqc}/{self.id}_clean_fastqc.html {self.p_report}/1.qc
fi
""")
...
""")
        ...
        if self.kind == 'bacteria' or self.kind == 'fungi':
            cmd.append(f"""# 4.anno
cp -r {self.p_VFDB}/show_virulence_gene.txt  {self.p_report}/4.anno
cp -r {self.p_VFDB}/show_virulence_gene.xlsx {self.p_report}/4.anno
cp -r {self.p_VFDB}/top1_virulence_gene.txt  {self.p_report}/4.anno
cp -r {self.p_VFDB}/top1_virulence_gene.xlsx {self.p_report}/4.anno
cp -r {self.p_VFDB}/viru.fa     {self.p_report}/4.anno
cp -r {self.p_VFDB}/viru_ref.fa {self.p_report}/4.anno

cp -r {self.p_drug}/detail_drug_resistance.txt  {self.p_report}/4.anno
cp -r {self.p_drug}/detail_drug_resistance.xlsx {self.p_report}/4.anno
cp -r {self.p_drug}/drug.fa {self.p_report}/4.anno
