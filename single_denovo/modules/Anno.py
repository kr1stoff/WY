...
card_des = os.path.join(DIR,'cfg/card.tab')
...
cd {self.p_VFDB}
time blastn -num_threads {int(th/2)} -evalue 1e-10 -query {self.predict_fa} -db {params.db_VFDB_nt}/VFDB -outfmt '6 qseqid qlen sseqid sgi slen pident length mismatch gapopen qstart qend sstart send evalue bitscore qcovs qcovhsp' -out {self.p_VFDB}/blast.out

if [ -f "{self.p_VFDB}/blast.out" ]; then
    {params.python3} {BIN}/4.anno/VFDB_filter_blastp.py {self.p_VFDB}/blast.out {params.db_VFDB_pro}/clean_VFDB_setB_des.txt {self.merge_file} {self.p_VFDB}
    {params.python3} {BIN}/tools/get_blast_top.py -i {self.p_VFDB}/show_virulence_gene.txt -n 1 -o {self.p_VFDB}/top1_virulence_gene.txt
    
    # Output fasta
    awk 'NR>1' {self.p_VFDB}/show_virulence_gene.txt |cut -f1 |sort |uniq >{self.p_VFDB}/show_virulence_gene.id
    {params.seqkit} grep -f {self.p_VFDB}/show_virulence_gene.id {self.predict_fa} >{self.p_VFDB}/viru.fa

    # Output ref fasta
    awk 'NR>1' {self.p_VFDB}/show_virulence_gene.txt |cut -f2 |sort |uniq >{self.p_VFDB}/ref.id
    {params.seqkit} grep -nrf {self.p_VFDB}/ref.id {params.db_VFDB_nt}/VFDB_setB_nt.fas >{self.p_VFDB}/viru_ref.fa
fi
""")
...
cd  {self.p_drug}
time rgi main -n {int(th/2)} --input_type contig --clean -i {self.predict_fa} --output_file {self.p_drug}/drug
cut -f 1 {self.p_drug}/drug.txt |cut -f1 -d ' '|sed 's/ORF_ID/Contig/g' |sed 's/_1//g' > {self.p_drug}/contig.id
cut -f 8,10,11,15,16,21 {self.p_drug}/drug.txt > {self.p_drug}/cut_drug.txt
paste {self.p_drug}/contig.id {self.p_drug}/cut_drug.txt |{params.csvtk} join -tf 'ARO' - {card_des} >{self.p_drug}/cut_id_drug.txt
{params.python3} {BIN}/4.anno/parse.rgi.py {self.p_drug}/cut_id_drug.txt {self.merge_file}

# Output fasta
{params.seqkit} grep -nrf {self.p_drug}/contig.id {self.predict_fa} >{self.p_drug}/drug.fa
""")
        common.cmd2shell(cmd,sh)
        return sh
