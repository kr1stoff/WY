...
process CAZY_PARSE1 {
  ...
  script:
  """
  csvtk join -t -L -f 1 ${dir_cazy}/hmmer.out ${params.db.CAZY_dir}/CAZyDB.07302020.fam-activities-hmm.txt | csvtk round -t -f 'Coverage' -n 2 | csvtk mutate -t -f 1 -p '^(\\D+)' -n 'kind' > CAZY.txt
