...
# 项目概述
$section->desc("$blank6 通过分离培养获得细菌株，进行建库测序，获得较为纯净的菌株序列。本项目对细菌株（样本编号为$sample）的Nanopore+Illumina测序数据进行分析，通过从头组装最终得到高质量的基因组序列，并对该序列进行基因预测及基因注释。");
...
# 2.1.1 ONT
$section->ssubmenu("Nanopore");
my @fig_tree = ("./1.qc/${sample}.raw.png", "./1.qc/${sample}.cln.png");
my @labels = qw(过滤前 过滤后);
Image($section, \@fig_tree, \@labels, "碱基质量图");
# 2.1.2 ILMN
$section->ssubmenu("Illumina");
my @figs1 = ("./1.qc/fastqc/before/${sample}.1_fastqc/Images/per_base_quality.png", "./1.qc/fastqc/after/${sample}.cln.1_fastqc/Images/per_base_quality.png");
my @names1 = ("read1", "read1");
my @figs2 = ("./1.qc/fastqc/before/${sample}.2_fastqc/Images/per_base_quality.png", "./1.qc/fastqc/after/${sample}.cln.2_fastqc/Images/per_base_quality.png");my @names2 = ("read2", "read2");
my @labels = qw(过滤前 过滤后);
Image($section, \@figs1, \@names1, "碱基质量图",\@figs2, \@names2, \@labels);
...
$section->ssubmenu("Illumina");
$section->tsv2html(
    -file   => "${dir}/1.qc/fastp/fastp_stats.tsv",
    -top => 100,
    -name   => "数据统计结果",
    -header => 1);
